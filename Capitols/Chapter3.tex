...
El resultat d'aquesta fase va ser una aplicació \textbf{completament funcional} a nivell de càlcul: el sistema ja era capaç de generar totes les cartes, mapes i gràfics necessaris, incloent-hi la complexa implementació tècnica del \textbf{``Ninot Espiritual''}. Aquesta visualització dinàmica, generada mitjançant la manipulació programàtica d'arxius SVG, va ser un dels reptes algorítmics més importants resolts en aquesta primera etapa. La robustesa del nucli matemàtic de NUMEN és fruit d'aquesta dedicació artesanal, on cada línia de codi va ser escrita i validada manualment, creant una base sòlida i lliure de ``caixes negres''.
...
\begin{itemize}
    \item \textbf{Infraestructura i persistència (Firestore):} Integració de \textbf{Google Firestore} per persistir estudis i habilitar la \textbf{visualització d'historial} al núvol, permetent recuperar i gestionar estudis antics directament des de l'aplicació.
    \item \textbf{Seguretat i autenticació (Login):} Implementació de \textbf{Firebase Authentication} i regles de seguretat per protegir la privacitat de les dades davant l'exposició pública, assegurant que només l'administrador tingui accés a informació sensible.
    \item \textbf{Refactorització de la impressió (Native PDF):} Reemplaçament de les ''captures de pantalla'' de la primera etapa per un sistema de generació de PDF natiu. Ara s'utilitzen plantilles vectorials que repliquen fidelment el disseny amb qualitat professional, independent de la resolució.
