...
Addicionalment, a l'arrel del projecte es troben carpetes clau per al cicle de desenvolupament:
\begin{itemize}
    \item \textbf{assets/}: Emmagatzema els recursos estàtics com les imatges, captures de pantalla per a la web i els fitxers \textbf{.svg} base per al ``Ninot Espiritual'' (\texttt{siluetaT.svg}, \texttt{siluetaCreu.svg}, etc.).
    \item \textbf{tools/}: Conté scripts d'utilitat en format \texttt{.bat} per automatitzar tasques repetitives, com \texttt{kill\_dart.bat} (per tancar processos Dart persistents) o \texttt{deploy\_prod.bat} per facilitar el desplegament a producció.
    \item \textbf{test/}: Directori dedicat a les proves automatitzades, incloent tests unitaris (\texttt{numerology\_service\_test.dart}) per validar la integritat dels algoritmes de càlcul.
\end{itemize}
...
Una de les funcionalitats clau sol·licitada per l'usuari era la visualització gràfica de l'estudi a través del ``Ninot Espiritual''. Aquest requisit plantejava un repte tècnic important degut a la gran variabilitat de la representació. Cal destacar que, metodològicament, aquesta figura correspon a la \textbf{1a Etapa} del motor numerològic, servint com a síntesi visual inicial abans d'aprofundir en els càlculs posteriors.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{Imatges/figma_structure.png}
        \caption{Estructura de capes (1-9 parts). limitant}
        \label{fig:figma_structure}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{Imatges/figma_layers.png}
        \caption{Detall de línies guia i simetria.}
...
La funcionalitat de ``Compartir Resultat'' ha requerit un disseny específic per potenciar l'experiència d'usuari (UX). L'objectiu principal és facilitar l'obtenció de feedback immediat del client final, eliminant barreres d'entrada: es permet l'accés directe a l'informe sense necessitat que l'usuari es registri o creï un compte. S'ha optat per enllaços temporals per mantenir un equilibri entre aquesta accessibilitat oberta i una política de dades neta, evitant l'acumulació indefinida d'informació pública.
