...
\begin{itemize}
    ...
    \item \textbf{IDE:} Visual Studio Code com a entorn de desenvolupament inicial. Posteriorment i durant l'etapa de PFG, s'ha utilitzat \textbf{Antigravity} (una versió avançada de VS Code amb IA agèntica integrada) per accelerar la refactorització i documentació del codi.
...
\begin{enumerate}
    \item Es descompon la data en Dia, Mes i Any.
    \item Es redueix cada component a un dígit (o nombre mestre).
    \item Es sumen els tres components reduïts.
    \item El resultat final es redueix novament si és necessari.
\end{enumerate}

\subsection{La Inclusió (The Inclusion)}
La Inclusió és una eina complexa d'autoconeixement que analitza la distribució de les lletres del nom complet en nou àrees o ``cases''. En aquest context, el terme ``Casa'' abstreu el concepte d'àrea vital (ex: Personalitat, Treball, Família), actuant com a contenidor lògic per als valors numèrics. Tècnicament, es tracta d'un histograma de freqüències on cada ``casa'' (de l'1 al 9) comptabilitza quantes vegades apareixen les lletres amb aquell valor numèric en el nom de la persona.
...
\begin{itemize}
    ...
    \item \textbf{Evolució:} Càlcul recursiu que segueix la cadena de referències. Si la Casa X té valor Y, el valor d'Evolució serà el contingut de la Casa Y. Això traça el vector de direcció de l'energia.
    \item \textbf{Inconscient:} Es calcula com el complement a 9 de l'habitant ($9 - Habitant[i]$), revelant el patró de comportament subjacent o la resposta instintiva oculta.
\end{itemize}

\subsection{Algorítmica del Nucli}
...
\begin{itemize}
    ...
    \item \textbf{Cicles de Vida:} Divideixen la vida en tres grans etapes, definides per la vibració dels components de la data:
    \begin{itemize}
        \item \textbf{Cicle Formatiu:} Correspon al \textbf{Mes} de naixement (reduït).
        \item \textbf{Cicle Productiu:} Correspon al \textbf{Dia} de naixement (reduït).
        \item \textbf{Cicle de Collita:} Correspon a l'\textbf{Any} de naixement (reduït).
    \end{itemize}
\end{itemize}

\subsubsection{Implementació de la Matriu de Cicles (Codi)}
Des d'una perspectiva tècnica, aquesta visualització no és una taula estàtica, sinó una matriu generada procedimentalment en temps d'execució. L'estructura de dades subjacent creua dues dimensions:
\begin{itemize}
    \item \textbf{Eix Y (Temporal):} Una llista iterativa d'Anys Personals $A_i$ calculats a partir de l'Any actual.
    \item \textbf{Eix X (Atributs):} El vector d'Habitants $H$ resultant de l'anàlisi del nom.
\end{itemize}

L'algorisme de renderitzat recorre aquesta graella i aplica una funció de relleu condicional basada en \textbf{aritmètica modular}. Això genera un efecte de ``carrusel'' infinit: quan la diagonal arriba a la columna 9, el següent any torna a activar la columna 1, creant un cicle continu.
\[ Index_{highlight} = (CV + Offset_{any}) \pmod 9 \]

En l'exemple següent es pot observar aquest comportament cíclic:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{Imatges/matriu_cicles.png}
    \caption{Captura real de la Matriu de Cicles a l'aplicació. S'observa la diagonal de ressonància (color salmó) i l'efecte cíclic de retorn.}
    \label{fig:cicles_matrix_app}
\end{figure}
...
La figura es descompon en regions (Cap, Braços, Cames, Tronc), i cadascuna està vinculada a coordenades numerològiques específiques. L'algoritme recorre el mapa d'Habitants i la Data de Naixement i, per a cada coincidència, ``activa'' la regió corresponent del gràfic.
