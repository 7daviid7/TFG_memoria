...
\chapter{Enginyeria de Prompts i Context Teòric}
\label{AppendixB}

Aquest annex conté el codi font complet del mètode \texttt{\_buildPrompt} utilitzat en el \texttt{GeminiService}. Aquest fragment és crític per entendre com s'ha mitigat el risc d'al·lucinacions de la IA, injectant un context teòric rígid i definicions estrictes (Mètode Coquatrix) que el model de llenguatge ha de respectar obligatòriament.

S'observa com el \textit{System Prompt} inclou:
\begin{itemize}
    \item \textbf{Rol:} Expert en Numerologia Evolutiva.
    \item \textbf{Context Teòric:} Definicions exactes de cada Número, Casa i Habitant.
    \item \textbf{Diccionari de Dades:} Guia per interpretar el JSON de sortida del càlcul.
    \item \textbf{Estructura de Resposta:} Instruccions de formatatge.
\end{itemize}

\begin{lstlisting}[language=Dart, caption=Codi complet de construcció del Prompt (GeminiService.dart)]
...
''';
    ...
    return '''
ROL:
Ets un expert consultor en Numerologia Evolutiva...
$contextTeoric
...
- "mapVida": Conté el ''Camino de Vid'' (Molt important) i els cicles de vida.
